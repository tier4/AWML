"""
Phase 1: Freeze PTv3 backbone, train PPS head only (~10 epochs).

Starting from model_best.pth (DefaultSegmentorV2 trained on T4 GT):
  - Backbone weights are loaded and frozen.
  - Prototypes are warm-started from seg_head.weight (no random init).
  - Only PartProtoHead parameters are updated.
...
# ── Experiment settings ──────────────────────────────────────────────────────
save_path = "exp/ptv3_pps_phase1"
mlflow_run_name = "ptv3_pps_phase1"
...
epoch = 10
eval_epoch = 10          # evaluate once at the end of phase 1
clip_grad = 10.0
sync_bn = False
enable_amp = False
empty_cache = False
...
# ── Model ────────────────────────────────────────────────────────────────────
# Provide the absolute path to the DefaultSegmentorV2 checkpoint.
# Backbone weights + seg_head.weight (prototype warm-start) are loaded from it.
...
model = dict(
    ...
    head=dict(
        ...
        # supervised_class_ids=None  → full supervision (all 26 classes)
        # To switch to partial supervision for rare classes only, uncomment:
        # supervised_class_ids=[13, 14, 18, 21, 22],  # forklift,kart,personal_mobility,traffic_cone,stroller
    ),
)

# ── Optimizer & Scheduler ────────────────────────────────────────────────────
# Phase 1: backbone is frozen, only head parameters are updated.
# Higher lr is fine since head starts from a warm prototype init.
...
# No param_dicts needed; backbone is frozen (no gradients).
param_dicts = []

# ── Data pipeline (same as PTv3 T4 config) ──────────────────────────────────
data = dict(
    num_classes=num_classes,
    ignore_index=ignore_index,
    train=dict(
