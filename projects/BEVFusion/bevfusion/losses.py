...
def corner_l1_losses(pred: Tensor, target: Tensor, norm_values: Optional[Tensor] = None, barrier_masks = None) -> Tensor:
    ...
    if barrier_masks is not None: 
        ...
        shifted_losses = (
            pred - target_shifted_180
        ).abs().sum(dim=-1).mean(dim=-1)  # [B, N]
...
class BEVCornerLoss(nn.Module):
    ...
    def _convert_to_bev_corners(
        ...
        # Diagonal values, (B, N,)
        lw_diagonal = torch.sqrt(
            lw[:, :, 0].square() + lw[:, :, 1].square() + 1e-6
        )
    ...
    def forward(
        ...
        preds_corners, _ = self._convert_to_bev_corners(
            bboxes=preds_tensor, labels=labels
        )
        gts_corners, gt_diagonal_values = self._convert_to_bev_corners(
            bboxes=gts_bboxes, labels=labels
        )
