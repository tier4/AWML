...
from projects.CenterPoint.models.dense_heads.centerpoint_head import Cus 
...
class BEVFusionImageRoiHead(nn.Module):
  def __init__(
        self,
				num_classes: int,
				in_channels: int = 256,
        ...
        **kwargs,
        ...
        super(CenterHeadBEVFusionImageRoiHead, self).__init__()
				self.num_classes = num_classes
				if ffn_head is None:
					ffn_head = dict(type='CustomSeparateHead', conv_cfg=dict('Conv1d'), norm_cfg=dict(type='BN1d'), bias=True)
				
				heads = dict(
					heatmap=(self.num_classes, 2)
				)
				self.ffn_head = MODELS.build(
					ffn_head
				)
