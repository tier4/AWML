...
class CustomResNet(nn.Module):
    ...
    def __init__(
        ...
        num_channels = (
            [numC_input * 2 ** (i + 1) for i in range(len(num_layer))] if num_channels is None else num_channels
        )
        ...
        if block_type == "BottleNeck":
            ...
            for i in range(len(num_layer)):
                ...
                layer.extend(
                    [Bottleneck(curr_numC, curr_numC // 4, norm_cfg=norm_cfg) for _ in range(num_layer[i] - 1)]
                )
