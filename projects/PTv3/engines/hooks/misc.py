...
class CheckpointLoader(HookBase):
    ...
    def before_train(self):
        ...
        if self.trainer.cfg.weight and os.path.isfile(self.trainer.cfg.weight):
            ...
            # A missing `_metadata` dict, e.g. from checkpoints not generated by mmdet3d, causes SparseModule._load_from_state_dict to incorrectly permute
            # the weights (it assumes spconv v1 format when `_metadata` is missing).
            # The fix: force `_metadata` to be present to ensure compatibility with all modern non-mmdet3d model checkpoints using spconv.
            weight._metadata = getattr(checkpoint["state_dict"], "_metadata", OrderedDict())
            weight._metadata["version"] = 2
