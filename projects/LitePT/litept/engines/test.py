...
from litept.datasets.builder import build_dataset
from litept.datasets.utils import collate_fn
from litept.models import build_model
...
class TesterBase:
    ...
    def visualize_results(self, outputs, result_path):
        ...
        coords, colors = get_segmentation_colors(
            segment, result_path, self.cfg.class_colors, self.logger
        )
        visualize_point_cloud(coords, colors, "Ground Truth")
        coords, colors = get_segmentation_colors(
            pred, result_path, self.cfg.class_colors, self.logger
        )
    ...
    def test(self):
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            result_save_path = os.path.join(
                save_path, "{}_{}_pred.npz".format(idx, data_name)
            )
            if os.path.isfile(pred_save_path):
                ...
                np.savez_compressed(
                    result_save_path, pred=pred, feat=feat.cpu().numpy()
                )
