...
class MultiBlockGenerator(object):
    ...
    def __call__(self, data_dict):
        ...
        full_point = self.local_transform(
        ...
        context_view_index = global_view_index[
            ~np.isin(global_view_index, local_view_index)
        ]
...
class MultiBlockGeneratorEarlyFuseDiffAug(object):
    ...
    def __call__(self, data_dict):
        ...
        global_view_index = global_views[0]["index"]
        local_view_index = [local_view["index"] for local_view in local_views]
        local_view_offset = np.cumsum([data.shape[0] for data in local_view_index])
        ...
        for local_view in local_views:
            ...
            local_view["index"] = np.arange(
                context_view_index.shape[0], local_view["coord"].shape[0]
            )
