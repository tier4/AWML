...
class PointCloudIntensityAnalysisCallback(AnalysisCallbackInterface):
    ...
    def _compute_scenario_voxel_counts(
        ...
        for scenario_data in analysis_data.scenario_data.values():
            ...
            for sample in selected_sample_data:
                ...
                intensities += list(points[:, 3]) if points.shape[1] > 3 else []
    ...
    def _compute_split_voxel_counts(
        ...
        for analysis_data in dataset_analysis_data.values():
            intensities += self._compute_scenario_voxel_counts(analysis_data)
    ...
    def run(self, dataset_split_analysis_data: Dict[DatasetSplitName, AnalysisData]) -> None:
        ...
        for split_option in SplitOptions:
            ...
            mean, std = self._compute_split_voxel_counts(dataset_analysis_data=dataset_voxel_data)
            print_log(
                f"Split: {split_option.value}, Mean Intensity: {mean:.2f}, Std: {std:.2f}"
            )
